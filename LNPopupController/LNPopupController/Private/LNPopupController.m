...
- (void)_transitionToState:(LNPopupPresentationState)state animated:(BOOL)animated useSpringAnimation:(BOOL)spring allowPopupBarAlphaModification:(BOOL)allowBarAlpha completion:(void(^)(void))completion transitionOriginatedByUser:(BOOL)transitionOriginatedByUser
{
	...
	if(_popupControllerState == LNPopupPresentationStateClosed)
	{
		...
		[UIView performWithoutAnimation:^{
			self->_currentContentController.view.frame = self->_containerController.view.bounds;
			self->_currentContentController.view.clipsToBounds = NO;
			self->_currentContentController.view.autoresizingMask = UIViewAutoresizingNone;
			
			if(CGColorGetAlpha(self->_currentContentController.view.backgroundColor.CGColor) < 1.0)
			{
			...
			[self.popupContentView.contentView addSubview:self->_currentContentController.view];
			[self.popupContentView.contentView sendSubviewToBack:self->_currentContentController.view];
...
{
	[UIView performWithoutAnimation:^{
		[self.popupBarStorage.progressView setProgress:self->_currentPopupItem.progress animated:NO];
	}];
}
